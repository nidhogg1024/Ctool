import {release, validateReleaseVersion, version} from "ctool-adapter-base";
...
(async () => {
    validateReleaseVersion(
        ...
        "tauri",
        version(),
        join(__dirname, "../src-tauri/tauri.conf.json5"),
    )
    const files: { path: string, name: string }[] = []

    const filesPush = (path: string, name: string) => {
        if (path === "" || !existsSync(path)) {
            throw new Error(`missing Tauri release artifact: ${name}`);
        }
    ...
    if (platform === "darwin") {
        ...
        if (!existsSync(appFile)) {
            throw new Error(`missing Tauri release artifact: mac${arch}_app`);
        }
    ...
    const expectedFileCount = platform === "win32" ? 1 : 2;
    if (files.length !== expectedFileCount) {
        throw new Error(`expected ${expectedFileCount} Tauri release artifacts for ${platform}, got ${files.length}`);
    }
    for (const file of files) {
